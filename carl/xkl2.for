...
C
        INTEGER BAS,BASX,SDAT,NSDAT,TNUM,NUMBER,Z,DAT,ELM
        INTEGER MIDV(4),NT
	DOUBLE PRECISION BASE
	COMMON /BASE/BASE(2,20)
C
...
C
        INTEGER TNUM,IA1,IA2,IA3,IA4,IA5,IA6,SDAT,IBF,IERT,IERC
...
44      IF (IERT.NE.7) GOTO 200
        TYPE 22
22	FORMAT(1X,'DATA BASE BUSY... WAITING 10 SECS.')
        CALL WAIT(8.0)
45      CALL DBUPD(-1)
        CALL DBFIND(2,1,TNUM,1,12,1,0)
        CALL DBNREC(NUMBER)
...
300     RETURN
        ...
        INTEGER TNUM,SITC(4),REPF(4),ME,TIM1,SDAT,NUMBER
	INTEGER IBF,IERT,IERC
	COMMON /IBF/IBF,IERT,IERC

	CALL OPENDB(16,$200,1)
        CALL DBFIND(2,1,TNUM,1,12,1,0)
        CALL DBNREC(NUMBER)
...
C
        INTEGER TNUM,NUMBER,NDAT,NTIM,STAT,MAINT,IBF,IERT,IERC
        INTEGER IPXNAM,SVER,F1,ME,NEWNAM,TDAT,TTIM
