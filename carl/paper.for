...
C
C
C       This program is the sole property and confidential information
C	of TYMSHARE, Inc.; TYMNET, Inc.; McDonnell Douglas Network
C	Systems Co.; British Telecom, plc; BT North America, Inc. and
C	MCI Telecommunications Corp. (Or whatever our name is this year).
...
998	format(1x,'Reached step #',20(i2,2x))
...
C	CALL DBSTRT(-1,1,-4,0,1,0,5,0,-1,0,-4,1,1,1,5,1)
C	CALL DBFOR
	CALL DBF6
C	CALL DBFX
	CALL DBERR($4930,IERT,IERC,IBF)
	CALL DBOPEN(BASE(1,20))
...
41      IF (FLAG.GT.0) NNET=VNET(FLAG)
	IF (FLAG.LT.1) NNET='  ALL  '
	IF (XX.LT.0) LIC=1
	ENCODE(841,207,DISC)
	I=0
	IF (SRT.LT.1) GOTO 59
47      TYPE 48
48	FORMAT(/1X,'ENTER "YOUR" INITIALS PLEASE : ',$)
	ACCEPT 49,ME
49	FORMAT(A3)
	CALL HISET(ME)
	DECODE(3,50,ME)M1,M2,M3
50	FORMAT(A1,A1,A1)
	IF ((M1.EQ.' ').OR.(M2.EQ.' ')) GOTO 47
	IF ((M1.ge.'A').AND.(M1.LE.'Z')) GOTO 56
	GOTO 47

56      CALL DBERR($4930,IERT,IERC,IBF)
C
C	DO THIS FIRST TIME FOR EACH USER -- PRINT ACCESS MESSAGE
C	IF ALREADY THROUGH, THEN SKIP.  IF NO MESSAGE FOR UNAME SKIP
C	OTHERWISE PRINT MESSAGE AND CONTINUE
C
	IF (ALREDY.EQ.5) GOTO 59
	ALREDY=ALREDY+1
	CALL DBCHNG(13,ALREDY)
	CALL DBOPEN('NOCLOSE',BASE(1,19))
	CALL DBSET(2)
	...
	CALL TYPINF(INFO,4)
C
C	HERE IS THE MAIN LOOP FOR COMMANDS
C
59      TYPE 60
60      FORMAT(/1X,'ENTER COMMAND... OR A',4H(CR),' FOR HELP : ',$)
61	format(1x,'Command: ',A5)
	ACCEPT 65,ANS
	CALL HISET(ANS)
65      FORMAT(A5)
	type 61,ans
...
264     CALL DBADD(1,TNUM,2,SDAT,3,TIM1,4,ME,'STAT',STATUS(L),
	...
	CALL DBCLOS
265     CALL DBERR($4930,IERT,IERC,IBF)
	ENCODE(320,233,INFO)
233     FORMAT(320(1H ))
...
651     TYPE 711,TIC
	ACCEPT 120,TNUM
9650    CALL DBOPEN(BASE(1,1),BASE(1,2),BASE(1,8))
	Z=1
...
652     FORMAT(/1X'THE TICKET YOU WANT IS IN THE ARCHIVE DATA BASE',
	...
	CALL DBOPEN(BASE(1,4),BASE(1,5),BASE(1,9))
	Z=2
	CALL DBSET(1)
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
	...
	CALL SETBAS(11,'B1',TNUM)
	CALL SETBAS(12,'B2',TNUM)
	CALL SETBAS(13,'B8',TNUM)
305     CALL DBERR($659,IERT,IERC,IBF)
	CALL DBOPEN(BASE(1,11),BASE(1,12),BASE(1,13))
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
	IF (NUMBER.GT.0) GOTO 670
659     TYPE 660,TNUM
...
686     IF (CDAT.EQ.0)GOTO 685
	...
	SY=SY-1900
6  CALL DBNDAT(DUP,JM,JD,JY)
	JY=JY-1900
687     IY=IY-1900
	CALL MTTR(SDAT,DUP,TIM1,TUP,ELM,HOUR)

	GOTO (654,655,674,688) NT
	NT=1

8650	FORMAT(/1X,'NETWORK',T11,2H: ,A10,T24,'SEVERITY',
     1  T37,2H: ,I1,T50,'CUSTOMER: ',3A5,/1X,A5,' TKT#: ',I8,
     2  T24,'STATE',T37,2H: ,A5,T50,'CONTACT : ',4A5,/1X,A6,
     3  T11,2H: ,I5,T24,'CONTROL GROUP: ',A5,T51,'CPHONE : ',
     4  4A5,/1X,A6,T11,2H: ,I5,T24,'REFERRED TO',T37,2H: ,A5,
     5  T50,'LOCALE',T58,2H: ,4A5)

8658	FORMAT(T24,'CLSCD : '2A5,T49,'BILL/T40: 'A3,1H/,A5,
     1  //1X,'REPORTED PROBLEM: ',2A5,/)

8659	FORMAT(1X,A9,': ',A3,T24,'ELAPS : ',F8.1,4H HR.,T50,
     1  'REF.TKT#: ',2A5,/1X,'OPEN BY  : ',A3,T24,'CLSCD : '2A5,T49,
     2  'BILL/T40: ',A3,1H/,A5,/1X,'ZONE',T11,2H: ,I1,
     3  /,/1X,'REPORTED PROBLEM: ',2A5,/)


654	TYPE 8650,NET,SVER,CNAM,NTTYPE(NT),TNUM,STAT,SITC,
     1  NTTEXT(NT),NODE,NETOP(HH),SITP,NTKTXT(NT),KHST,MAINT,LOC
	
	TYPE 680,PROD,V1,V2,V3,RNAM,VER,YN(ESC),RFON,
     2  SRGN,HQCC,COV,HRGN,V4,V6,STH,EHR,YN(PRO),ID,
     3  AM(IM),IY,TIM1,TECH,INT,JD,AM(JM),JY,TUP,CKT,F1,SD,AM(SM),
     4  SY,TCLSD,TRB,HOUR,ONUM
	TYPE 8658,COC,YN(BILL),YN(T40),PROBL
	GOTO 684

680     FORMAT(1X,'PRODUCT',T11,2H: ,A5,T24,'MGMT ESCAL',T37,
     1  3H: H,I1,2H S,I1,2H T,I1,T50,'RPTD BY : ',4A5,
     2  /1X,'S/W VERS : ',A5,T24,'TECH ESCAL',T37,2H: ,A3,
     3  T51,'RPHONE : ',4A5,/1X,'S/W DIST : ',A5,
     9  T24,'DE-ESCAL CODE: ',I3,T50,'PLAN #',T58,2H: ,A2,
     A  /1X,'H/W DIST : ',A5,T24,'DEFER :  ',I3,1H/,I3,
     B  T50,'SITE HR ',2H: ,A5,' TO ',A5,
     X  /1X,'PRIMARY  : ',A3,T24,'OPEN  : ',
     C  I2,1H-,A3,1H-,I2,1X,A5,1HZ,2X,'COM SPEC: ',3A5,
     D  /1X,'OPEN BY',T11,2H: ,A3,T24,'RESLV : ',I2,1H-,A3,1H-,I2,
     E  1X,A5,1HZ,T50,'CKT #',T58,2H: ,4A5,/1X,'ZONE',T11,2H: ,I1,T24,
     F  'CLOSE : ',I2,1H-,A3,1H-,I2,1X,A5,1HZ,T50,'TELCO # : ',4A5,
     G  /,T24,'ELAPS :  ',F8.1,4H HR.,T50,'REF.TKT#: ',2A5)


655	TYPE 8650,NET,SVER,CNAM,NTTYPE(NT),TNUM,STAT,SITC,
     1  NTTEXT(NT),NODE,NETOP(HH),SITP,NTKTXT(NT),KHST,MAINT,LOC

	TYPE 689,HOST,V1,V2,V3,RNAM,SLOT,YN(ESC),RFON,
     2  PORT,HQCC,COV,PROD,V4,V6,STH,EHR,VER,ID,AM(IM),
     3  IY,TIM1,TECH,SRGN,JD,AM(JM),JY,TUP,CKT,HRGN,
     4  SD,AM(SM),SY,TCLSD,TRB
	TYPE 8659,STEXT(1),YN(PRO),HOUR,ONUM,
     1  INT,COC,YN(BILL),YN(T40),F1,PROBL
	GOTO 684

689     FORMAT(1X,'HOST #',T11,2H: ,I6,T24,'MGMT ESCAL',T37,
     1  3H: H,I1,2H S,I1,2H T,I1,T50,'RPTD BY : ',4A5,
     2  /1X,'SLOT     : ',A2,T24,'TECH ESCAL',T37,2H: ,A3,
     3  T51,'RPHONE : ',4A5,/1X,'# PORTS  : ',I3,T24,
...
C
674	TYPE 8650,NET,SVER,CNAM,NTTYPE(NT),TNUM,STAT,SITC,
     1  NTTEXT(NT),NODE,NETOP(HH),SITP,NTKTXT(NT),KHST,MAINT,LOC

	TYPE 669,HOST,V1,V2,V3,RNAM,VER,YN(ESC),RFON,
     2  PROD,HQCC,COV,HRGN,V4,V6,STH,EHR,YN(PRO),
     3  ID,AM(IM),IY,TIM1,CKT,INT,JD,AM(JM),JY,TUP,TRB,F1,SD,AM(SM),
     4  SY,TCLSD,ONUM,HOUR,TECH
	TYPE 8658,COC,YN(BILL),YN(T40),PROBL
	GOTO 684

669     FORMAT(1X,'HI NODE',T11,2H: ,I5,T24,'MGMT ESCAL',T37,
     1  3H: H,I1,2H S,I1,2H T,I1,T50,'RPTD BY : ',4A5,
     2  /1X,'HI KHOST : ',A5,T24,'TECH ESCAL',T37,
     ...
     F  'CLOSE : ',I2,1H-,A3,1H-,I2,1X,A5,1HZ,T50,'REF TKT#: ',2A5,
     G  /,T24,'ELAPS : ',F8.1,4H HR.,T50,'ANALYSIS: ',3A5)


688	TYPE 8650,NET,SVER,CNAM,NTTYPE(NT),TNUM,STAT,SITC,
     1  NTTEXT(NT),NODE,NETOP(HH),SITP,NTKTXT(NT),KHST,MAINT,LOC

	TYPE 99689,HOST,V1,V2,V3,RNAM,SLOT,YN(ESC),RFON,
     2  PORT,HQCC,COV,PROD,V4,V6,STH,EHR,VER,ID,AM(IM),
     3  IY,TIM1,TECH,SRGN,JD,AM(JM),JY,TUP,CKT,HRGN,
     4  SD,AM(SM),SY,TCLSD,TRB
	TYPE 8659,STEXT(2),YN(PRO),HOUR,ONUM,
     1  INT,COC,YN(BILL),YN(T40),F1,PROBL
	GOTO 684

99689   FORMAT(1X,'CARD #',T11,2H: ,I6,T24,
...
C
C	TYPE SO MANY STATUS BLOCK FOR A GIVEN TICKET
C
712     CALL DBSET(2)
...
C
C	CLEAR THE INFO ARRAY ... ??? 95 BYTES SHOULD BE 16 * 4 LINES
C	                         OR 16*5*4 TO BE CORRECT ..... ???
	ENCODE(320,233,INFO)
933     FORMAT(95(1H ))
C
C********************
...
777     FORMAT(/3X,'TIC#',1X,'DATE',2X,'WHO',3X,'NODE',2X,'CIRCUIT ',
	...
	IY=IY-1900
	CALL MTTR(SDAT,DUP,TIM1,TUP,ELM,HOUR)
	TYPE 789,TNUM,IM,ID,PROD,NODE,CKT,SVER,HOST,STAT,MAINT,
     1  NETOP(HH),YY(ESC),HOUR
789	FORMAT(1X,I6,1X,I2,1H/,I2,1X,A5,1X,I5,1X,4A5,1X,I1,1X,I6,
...
802     CALL DBOPEN(BASE(1,1),BASE(1,2))
	CALL DBSET(1)
...
1360    TYPE 1415,NUMBER
	GOTO 1400
1370    TYPE 1375
1400    CALL DBOPEN(BASE(1,1),BASE(1,4))
...
1427    CALL DBERR($4930,IERT,IERC,IBF)
	IF (N.EQ.5) GOTO 138
	GOTO 4950
C
...
2000    K=14
	I=1
	CALL DBOPEN(BASE(1,1),BASE(1,2),BASE(1,8),
	1           BASE(1,4),BASE(1,5),BASE(1,9))
...
C
2005    ENCODE(10,2003,FILE)ATTN(M)
2003    FORMAT(A5,'.OUT')
	OPEN(unit=8,device='DSK',access='SEQOUT',file=FILE)
C	OPEN(8,FILE,OUTPUT)
	CALL DBSET(1)
...
2060    CALL DBSET(1)
	...
	CALL SETBAS(12,'B2',TNUM)
	CALL SETBAS(13,'B8',TNUM)
	CALL DBERR($20659,IERT,IERC,IBF)
	CALL DBOPEN(BASE(1,11),BASE(1,12),BASE(1,13))
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
	IF (NUMBER.GT.0) GOTO 2080
20659   TYPE 660,TNUM
	...
	TYPE 665
	GOTO 4950
2080    IF (TNUM.LT.100000) ENCODE(10,2086,FILE)TNUM
...
2386    FORMAT(1X,'ENTER ITEM NUMBER : ',$)
	...
	GOTO (2384,2384,2384,2414,2415,2416,2417,2418,2419
     1  ,2423,2424,2427,2377,2377,2377,2377,2377,2377,2434
     2  ,2377,2437,2438,2377,2377,2377,2377,2443,2444,2377
     3  ,2377,2377,2448,2418,2419,5000,5000) N

2377	IF ((FLAG.GE.0).AND.(N.GT.1)) GOTO 2382
	GOTO (2384,2384,2384,2414,2415,2416,2417,2418,2419
     1  ,2423,2424,2427,2428,2429,2430,2431,2432,2433,2434
     2  ,2436,2437,2438,2439,2440,2441,2442,2443,2444,2445
     3  ,2446,2447,2448,2418,2419,5000,5000) N
	GOTO 2381
...
2414	CALL CHA4A5(1,10,43,7,'CIRCUIT # ')
	GOTO 2381
2415	CALL CHA4A5(1,10,44,8,'TELCO TRB#')
	GOTO 2381
2416	CALL CHA4A5(1,10,12,1,'SITE CONT.')
	GOTO 2381
2417	CALL CHA4A5(1,10,13,2,'SITE PHONE')
	GOTO 2381
2418	CALL CH1I(1,10,40,4,'NODE/LOWND')
	GOTO 2381
2419	CALL CH1I(1,10,42,6,'HOST/HINOD')
	GOTO 2381
2423	CALL CHA4A5(1,10,41,5,'SLOT      ')
	GOTO 2381
2424	CALL CHA4A5(1,10,7,0,'REPRT BY  ')
	GOTO 2381
2427	CALL CHA4A5(1,10,8,0,'CALL BACK ')
	GOTO 2381
2428	CALL CHHN1(1,TNUM,ME,STAT,UNAM)
	GOTO 2381
2429	CALL CHDUP1(1)
	GOTO 2381
2430	CALL CHTUP1
...
2432	CALL CHTDWN
	GOTO 2381
2433	CALL CH1I(1,10,54,0,'ELAPSED MN')
	GOTO 2381
2434	CALL CHA4A5(1,10,25,0,'COMMENT   ')
	GOTO 2381
2436	CALL CHA4A5(1,10,19,0,'REFERENCE#')
	GOTO 2381
2437	CALL CHA4A5(1,10,31,10,'CUSTOMER  ')
	GOTO 2381
2438	CALL CHA4A5(1,10,32,9,'TECH/ANAL')
	GOTO 2381
2439    CALL CH1I(1,10,2,13,'PRODUCTION')
	GOTO 2381
2440    CALL CH1I(1,10,3,23,'TOP 40    ')
	GOTO 2381
2441    CALL CH1I(1,10,45,0,'BILLABLE  ')
	GOTO 2381
2442    CALL CH1I(1,10,30,16,'PORTS     ')
	GOTO 2381
2443    CALL CHA4A5(1,10,22,21,'LOCATION  ')
	GOTO 2381
2444    CALL CH1I(1,10,23,11,'KERNAL HST')
	GOTO 2381
2445    CALL CHA4A5(1,10,24,12,'CONTRACT  ')
	GOTO 2381
2446    CALL CHASTH(1,10)
	GOTO 2381
2447    CALL CHAEHR(1,10)
	GOTO 2381
2448    CALL CHA4A5(1,10,33,22,'CODE VER#.')
	GOTO 2381
...
23999   IF (N.NE.1) GOTO 2401
	IF (UNAM(1).NE.'TELEC') CALL RESCOD(TNUM,CCC,ANS,L)
	IF ((M.EQ.41).OR.(M.EQ.36).OR.(M.EQ.21).OR.(M.EQ.25)
     1  .OR.(M.EQ.29).OR.(M.EQ.30).OR.(M.EQ.72).OR.(M.EQ.81).OR.
     2  (M.EQ.71)) ESC=2

2401	CALL OPENDB(1,$4955,1)
	CALL DBFIND(1,1,TNUM)
	IF ((M.EQ.41).OR.(M.EQ.36).OR.(M.EQ.21).OR.(M.EQ.25)
     1  .OR.(M.EQ.29).OR.(M.EQ.30).OR.(M.EQ.72).OR.(M.EQ.81).OR.
     2  (M.EQ.71)) ESC=2
...
2400    CALL DBCLOS
2402    CALL DBERR($4930,IERT,IERC,IBF)
...
2460    FORMAT(/1X,'THIS WILL TAKE A TICKET NUMBER FROM THE OLD DATA',
	...
	ACCEPT 120,TNUM
2461    TYPE 2464,BASE(1,4),BASE(2,4),BASE(1,1),BASE(2,1)
2464    FORMAT(1X,'SEARCHING ',2A10,'FOR RECORDS TO MOVE TO ',2A10)

	CALL OPNDB2(4,1,$4955,1,1)
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
...
2468    CALL DBCLOS
	CALL DBCLOS
	TYPE 2464,BASE(1,5),BASE(2,5),BASE(1,2),BASE(2,2)
	CALL OPNDB2(5,2,$4955,1,1)
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
...
2475    CALL DBCLOS
	...
	TYPE 2464,BASE(1,7),BASE(2,7),BASE(1,6),BASE(2,6)
	CALL OPNDB2(7,6,$4955,1,1)
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
...
93247   FORMAT(/1X,'TNUM',6X,'SEV',2X,'STATUS',2X,'CTL GRP',5X,
	...
	ISSTAR=' '
	IF(J.GT.0) ISSTAR='*'
	TYPE 93251,TNUM,SVER,STAT,NETOP(HH),CNAM,MAINT,HOUR,ISSTAR
93251   FORMAT(1X,I8,3X,I1,3X,A5,3X,A5,3X,3A5,3X,A5,5X,F7.1,3X,A1)
93245   CONTINUE
...
3294    CALL DBOPEN(BASE(1,1),BASE(1,2),BASE(1,8))
	GOTO 653
3296    TYPE 170,TIC
	CALL DBOPEN(BASE(1,1),BASE(1,2),BASE(1,8))
	ACCEPT 120,TNUM
...
3385    CALL DBGREC($3389,J)
	...
	TYPE 789,TNUM,IM,ID,PROD,NUD,CKT,SVER,HOST,STAT,MAINT,
     1  NETOP(HH),YY(ESC)
...
C
C       ROUTINE TO LIST CFSS TICKETS  [63]
C
3586    CALL DBOPEN(BASE(1,1))
...
C
3765    CALL DBOPEN(BASE(1,17),BASE(1,1))
...
3810    FORMAT(/15X,'PERFORM AN ESCALATION',/,/3X,'ROUTINE TO ',
	...
	CALL GDIS(V1,V2,V3,V4,V5,V6,TNUM,1)
...
4016    CALL DBERR($4015,IERT,IERC,IBF)
	SKIP=1
	GOTO 4018
4015    IF (IERT.NE.7) GOTO 4955
	SKIP=SKIP+1
	IF(SKIP.EQ.10) GOTO 40171
	TYPE 1379
	CALL WAIT(5.0)

4018    CALL DBOPEN(BASE(1,10))
	CALL DBUPD(-1)
	TYPE 4019
...
40171	CALL DBERR($4930,IERT,IERC,IBF)
	TYPE 40172
40172	FORMAT(/1X,'PROFIL WAS NOT UPDATED DUE TO DATABASE BUSY',
     1  ', CONTINUING...',/1X)
40170	SKIP=1
	IF ((ANS.EQ.'A2').OR.(ANS.EQ.'A ')) GOTO 4020
C	IF ((Z.EQ.3).AND.(NOD1.GT.0)) GOTO 3360
	IF ((Z.EQ.3).AND.(NOD1.GT.0)) GOTO 3959
	GOTO 59
C
...
4152	CALL DBADD(3,VNET(K),4,NODE,13,1,17,WELL,23,1,6,HOST,7,CKT,
...
4305	CALL OPNDB2(6,6,$4930,1,0)
	CALL DBFIND('MAINT',1,ATTN(P),1,'RS',1,0)
	CALL DBNREC(NUMBER)
	IF (NUMBER.GT.0) GOTO 4310

C       NO TICKETS OVER TO THIS GROUP OUTPUT SAME AND GOTO MAIN  PROMPT
	TYPE 2596,ATTN(P)
	GOTO 4950
...
4337    TYPE 220,ST
	...
	IF (NLINE.EQ.0) GOTO 4343
4342	CALL OPENDB(2,$4930,1)
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(ORDER)
	ORDER=ORDER+1
...
4375    FORMAT(1X,'ROUTINE TO FIND A TICKET NUMBER IN ALL THE DATA',
	...
	TYPE 4377,BASE(1,1),BASE(2,1)
4377    FORMAT(1X,'SEARCHING ',2A10,T40,$)
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
	TYPE 4379,NUMBER
4379    FORMAT('+',I3,' RECORDS FOUND')

	TYPE 4377,BASE(1,2),BASE(2,2)
	CALL DBOPEN(BASE(1,2))
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
	TYPE 4379,NUMBER

	TYPE 4377,BASE(1,6),BASE(2,6)
	CALL DBOPEN(BASE(1,6))
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
	TYPE 4379,NUMBER

	TYPE 4377,BASE(1,8),BASE(2,8)
	CALL DBOPEN(BASE(1,8))
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
	TYPE 4379,NUMBER

	TYPE 4377,BASE(1,4),BASE(2,4)
	CALL DBOPEN(BASE(1,4))
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
	TYPE 4379,NUMBER

	TYPE 4377,BASE(1,5),BASE(2,5)
	CALL DBOPEN(BASE(1,5))
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
	TYPE 4379,NUMBER

	TYPE 4377,BASE(1,7),BASE(2,7)
	CALL DBOPEN(BASE(1,7))
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
	TYPE 4379,NUMBER

	TYPE 4377,BASE(1,9),BASE(2,9)
	CALL DBOPEN(BASE(1,9))
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
	TYPE 4379,NUMBER
	...
	TYPE 4377,BASE(1,11),BASE(2,11)
	CALL DBOPEN(BASE(1,11))
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
	TYPE 4379,NUMBER

	TYPE 4377,BASE(1,12),BASE(2,12)
	CALL DBOPEN(BASE(1,12))
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
	TYPE 4379,NUMBER

	TYPE 4377,BASE(1,13),BASE(2,13)
	CALL DBOPEN(BASE(1,13))
	CALL DBFIND(1,1,TNUM)
	CALL DBNREC(NUMBER)
	TYPE 4379,NUMBER
...
4398    ENCODE(10,4400,FILE)NDATE
	TYPE 4406,FILE
4406    FORMAT(' ENTER FILE NAME (10 CHAR. MAX) (',A10,') : ',$)
	ACCEPT 4403,FILE
4403    FORMAT(A10)
	IF (FILE.EQ.'          ') ENCODE(10,4400,FILE)NDATE
4400    FORMAT(I5,'.OUT')
	OPEN(unit=8,device='DSK',access='SEQOUT',file=FILE)
C	OPEN(8,FILE,OUTPUT)
...
4912    FORMAT(/3X,'THERE ARE ',I3,' TICKETS OVER TO ',A5,1H.,//1X,
	...
	CALL DBVAL(2,SDAT,3,TIM1,'UNAM',PNAM)
	IF (RECORD.LT.2) GOTO 4914
	IF ((PNAM(1).NE.'NETCO').AND.(PNAM(1).NE.'NSSC').AND.
     1  (PNAM(1).NE.'NCF').AND.(PNAM(1).NE.'CSS80')) GOTO 4914
...
5101    FORMAT(/5X,'THIS ROUTINE WILL PRINT A LIST OF TICKETS FROM ',
	...
	TYPE 5105
5105    FORMAT(
     1  /1X,'TICKET   ORIGINATING               CURRENT   CURRENT',
     2  /1X,'NUMBER   CONT. GROUP   INITIALS    CONT.GRP  REF.GRP',
     3  '    STATUS',/1X)
	DO 5109 I=1,NUMBER
	CALL DBGREC($4930)
	CALL DBVAL(1,TNUM,4,F2,10,WHO,53,HN,51,MAINT,15,STAT)
	TYPE 5106,TNUM,NETOP(F2),WHO,NETOP(HN),MAINT,STAT
C	REMOVED '/' ON 5106 -- NO NEED TO DOUBLE SPACE THESE DAYS --
...
4930	CALL DBERR($4932,IERT,IERC,IBF)
	CALL DBNSET(NUMBER)
...
C	CALL DBFX
	TYPE 4933
4933	FORMAT(1x,'All Channels RELEASED here...')
	GOTO 59
...
99999   CALL DBEND
	...
	SUBROUTINE OPENDB(BAS,$,UPD)
	INTEGER BAS,UPD,IBF,IERT,IERC,COUNT
	REAL HANGON
	DOUBLE PRECISION BASE
	COMMON /IBF/IBF,IERT,IERC
	COMMON /BASE/BASE(2,20)
	COMMON /HANGON/HANGON(9)
	DATA HANGON/5.0,5.0,10.0,10.0,15.0,15.0,30.0,30.0,60.0/

	CALL DBERR($100,IERT,IERC,IBF)
	COUNT=0
	GOTO 200
100     IF (IERT.NE.7) RETURN 1
	COUNT=COUNT+1
	TYPE 150,HANGON(COUNT)
	IF (COUNT .GE. 9) COUNT=1
150	FORMAT(1X,'PLEASE STANDBY, DATABASE BUSY, WAITING...(',F3.0,')')
	CALL WAIT(HANGON(COUNT))
200     CALL DBOPEN(BASE(1,BAS))
	IF (UPD.NE.0) CALL DBUPD(-1)
	RETURN
	END
	SUBROUTINE OPNDB2(BAS1,BAS2,$,UP1,UP2)
	INTEGER BAS1,BAS2,UP1,UP2,IBF,IERT,IERC,COUNT
	REAL HANGON
	DOUBLE PRECISION BASE
	COMMON /IBF/IBF,IERT,IERC
	COMMON /BASE/BASE(2,20)
...
100     IF (IERT.NE.7) RETURN 1
	...
	TYPE 150,HANGON(COUNT)
	IF (COUNT .GE. 9) COUNT=1
150	FORMAT(1X,'PLEASE STANDBY, DATABASES BUSY. WAITING...(',F3.0,')')
	CALL WAIT(HANGON(COUNT))
200     CALL DBOPEN(BASE(1,BAS1),BASE(1,BAS2))
	CALL DBSET(2)
	IF (UP2.NE.0) CALL DBUPD(-1)
	CALL DBSET(1)
	RETURN
	END
   Q@ ½