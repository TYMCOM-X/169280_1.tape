...
115     FORMAT(/1X,'THE CURRENT CONTROL GROUP FOR TICKET NUMBER ',I8,
     1  ' IS... ',//5X,A5,/)
	TYPE 117,(NETOP(I),I=1,NUMCG)
117	FORMAT(8X,5(2X,I2,' = ',A5))
	TYPE 118
...
C
        INTEGER BAS,BASX,DUP,DAT,TNUM,NUMBER,Z,IERT,IERC,ELM
	DOUBLE PRECISION BASE
	COMMON /BASE/BASE(20)
...
118     FORMAT(1X,'DATA BASE BUSY... WAITING 8 SECOUNDS')
        CALL WAIT(8.0)
115     TYPE 117,BASE(BAST)
117     FORMAT(1X,'OPENING ',A10,' FOR UPDATE',$)
        CALL DBOPEN(BASE(BAST))
        CALL DBUPD(-1)
...
140     FORMAT('+','...CHANGE IS COMPLETE')
        IF (ATB2.EQ.0) RETURN
        CALL DBERR($150,IERT,IERC,IBF)
        GOTO 115

150     IF (IERT.NE.7) GOTO 295
        TYPE 118
118     FORMAT(1X,'DATA BASE BUSWAITING 8 SECOUNDS')
        CALL WAIT(8.0)
115     TYPE 117,BASE(BAST)
117     FORMAT(1X,'OPENING ',A10,' FOR UPDATE',$)
        CALL DBOPEN(BASE(BAST))
...
100     CALL DBERR($110,IERT,IERC,IBF)
        GOTO 115
